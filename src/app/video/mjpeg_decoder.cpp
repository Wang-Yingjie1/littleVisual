/*
 * @Author: @ClimbSnail
 * @Date: 2022-04-02 23:45:17
 * @LastEditors: WYJ
 * @LastEditTime: 2022-04-03 00:02:21
 * @FilePath: \littleVisual\src\app\video\mjpeg_decoder.cpp
 * @Description: 
 */
...
bool MjpegPlayDocoder::tft_output(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t *bitmap)
{
    ...
    if (m_isUseDMA)
    {
        ...
        screen.pushImageDMA(x, y, w, h, bitmap, dmaBufferPtr); // Initiate DMA - blocking only if last DMA is not complete
                                                             // The DMA transfer of image block to the TFT is now in progress...
    }
    else
    {
        // Non-DMA blocking alternative
        screen.pushImage(x, y, w, h, bitmap); // Blocking, so only returns when image block is drawn
    }
...
MjpegPlayDocoder::MjpegPlayDocoder(File *file, bool isUseDMA)
{
    ...
    m_tftSwapStatus = screen.getSwapBytes();
...
MjpegPlayDocoder::~MjpegPlayDocoder(void)
{
    ...
    screen.setSwapBytes(m_tftSwapStatus);
    // 释放资源
    video_end();
}
...
bool MjpegPlayDocoder::video_start()
{
    ...
    {
        m_displayBuf = (uint8_t *)malloc(MOVIE_BUFFER_SIZE);
        screen.setAddrWindow((screen.width() - VIDEO_WIDTH) / 2,
                           (screen.height() - VIDEO_HEIGHT) / 2,
